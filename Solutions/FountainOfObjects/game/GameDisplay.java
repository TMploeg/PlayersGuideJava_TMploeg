...
public class GameDisplay {
  ...
  private void displayGeneralIntro() {
    ConsoleHelper.printColor(
        "You enter the fountain of objects, a maze of rooms filled with dangerous pits in search of"
            + " the ",
        INTRO_COLOR);
    ConsoleHelper.printColor("Fountain of Objects", FOUNTAIN_COLOR);
    ConsoleHelper.printlnColor(".", INTRO_COLOR);

    ConsoleHelper.printlnColor(
        "Light is only visible in the entrance, and no other light is seen anywhere in the"
            + " caverns.",
        INTRO_COLOR);

    ConsoleHelper.printlnColor(
        "You must navigate the Caverns with your other senses.", INTRO_COLOR);
  }

  private void displayPitsIntro() {
    ConsoleHelper.printColor("Look out for ", INTRO_COLOR);
    ConsoleHelper.printColor("pits", PIT_COLOR);
    ConsoleHelper.printColor(". You will feel a breeze if a ", INTRO_COLOR);
    ConsoleHelper.printColor("pit", PIT_COLOR);
    ConsoleHelper.printlnColor(" is in an adjacent room.", INTRO_COLOR);

    ConsoleHelper.printColor("If you enter a room with a ", INTRO_COLOR);
    ConsoleHelper.printColor("pit", PIT_COLOR);
    ConsoleHelper.printlnColor(", you will die.", INTRO_COLOR);
  }

  private void displayMaelstromIntro() {
    ConsoleHelper.printColor("Maelstroms", entityService.getEntityColor(EntityType.MAELSTROM));
    ConsoleHelper.printlnColor(
        " are violent forces of sentient wind. Entering a room with one could transport you to any"
            + " other location in the caverns.",
        INTRO_COLOR);

    ConsoleHelper.printlnColor(
        "You will be able to hear their growling and groaning in nearby rooms.", INTRO_COLOR);
  }

  private void displayAmarokIntro() {
    ConsoleHelper.printColor("Amaroks", entityService.getEntityColor(EntityType.AMAROK));
    ConsoleHelper.printlnColor(
        " roam the caverns. Encountering one is certain death, but you can smell their rotten"
            + " stench in nearby rooms.",
        INTRO_COLOR);
  }
