...
public class Game {
  ...
  private void moveMultiple(Point movementOffset) {
    Optional<Direction> horizontalDirection =
        switch (Integer.signum(movementOffset.x)) {
    ...
    Optional<Direction> verticalDirection =
        switch (Integer.signum(movementOffset.y)) {
    ...
    if (result == ShootResult.HIT) {
      Room adjacentRoom =
          map.getCurrentRoom()
              ...
              .orElseThrow(() -> new NullPointerException("room should exists"));

      EntityType target =
          adjacentRoom
              .getEntity()
              .orElseThrow(() -> new NullPointerException("entity should exist"));
