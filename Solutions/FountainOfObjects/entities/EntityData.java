...
class EntityData {
	private final static Map<EntityType, EntityData> entityDataMap = new HashMap<>(){{
		put(
		  EntityType.PLAYER,
		  new EntityData(
			null,
		...
		put(
		  ...
		  new EntityData(
		    new HashMap<MessageType, String>(){{
			  ...
			  put(MessageType.INTERACT, "You where blown away by a " + EntityType.MAELSTROM.toString().toLowerCase() + ".");
			}},
			ConsoleColor.TEAL
		  )
		);
		put(
		  ...
		  new EntityData(
			new HashMap<MessageType, String>(){{
			  ...
			  put(MessageType.INTERACT, "You where mauled by an " + EntityType.AMAROK.toString().toLowerCase() + ".");
			}},
			ConsoleColor.KUMERA
		  )
		);
